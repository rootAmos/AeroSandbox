...
# Load airfoils from the local airfoils directory
# Use __file__ to get the script's directory, not the current working directory
script_dir = Path(__file__).parent
airfoils = {
    name: asb.Airfoil(
        name=name,
        coordinates=script_dir / "airfoils" / f"{name}.dat"
    )
    for name in [
        "ag34",
        "ag35",
...
if __name__ == '__main__':
    ...
    if make_plots:
        ...
        ##### Section: Geometry
        print("Generating three-view plot...")
        airplane.draw_three_view(show=False)
        print(f"Saving three-view plot to: {script_dir / 'figures' / 'three_view.png'}")
        p.show_plot(tight_layout=False, savefig=script_dir / "figures" / "three_view.png")
        print("Three-view plot saved successfully!")

        ##### Section: Mass Budget
        print("Generating mass budget plot...")
        ...
        print(f"Saving mass budget plot to: {script_dir / 'figures' / 'mass_budget.png'}")
        p.show_plot(savefig=script_dir / "figures" / "mass_budget.png")
        print("Mass budget plot saved successfully!")
